...
export class PeopleKB {
   ...
   */
  async embed(docs: Person[]): Promise<void> {
   ...
   */
  async query({ query, topK = 5 }: { query: string; topK?: number }): Promise<QueryResult[]> {
   ...
   */
  getStats(): { total: number; genres: Record<string, number> } {
